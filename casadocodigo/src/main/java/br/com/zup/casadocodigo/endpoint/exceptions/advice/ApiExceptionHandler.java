...
import br.com.zup.casadocodigo.endpoint.exceptions.EntidadeEmUsoException;
import br.com.zup.casadocodigo.endpoint.exceptions.EntidadeNaoEncontradaException;
import br.com.zup.casadocodigo.endpoint.exceptions.NegocioException;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	private ResponseEntity<Object> handleValidationInternal(Exception ex, HttpHeaders headers, HttpStatus status,
		...
		List<Problem.Object> problemObjects = bindingResult.getAllErrors().stream().map(objectError -> {
			...
			return Problem.Object.Builder.newBuilder().name(name).userMessage(message).build();
	...
	protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
			HttpHeaders headers, HttpStatus status, WebRequest request, Throwable throwable) {
	 ...
	 * 
	 * HttpStatus status = HttpStatus.FORBIDDEN; ProblemType problemType =
	 * ProblemType.ACESSO_NEGADO; String detail = ex.getMessage();
	 * 
	...
	protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
		...
		if (body == null) {
			body = Problem.Builder.newbuilder().timestamp(OffsetDateTime.now()).title(status.getReasonPhrase())
					.status(status.value()).userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL).build();
		} else if (body instanceof String) {
			body = Problem.Builder.newbuilder().timestamp(OffsetDateTime.now()).title((String) body)
					.status(status.value()).userMessage(MSG_ERRO_GENERICA_USUARIO_FINAL).build();
		}
	...
	private Problem.Builder createProblemBuilder(HttpStatus status, ProblemType problemType, String detail) {
		...
		return Problem.Builder.newbuilder().timestamp(OffsetDateTime.now()).status(status.value())
